...
而 `ProcessManager` 和内核中代表进程的Process类型之间的衔接桥梁是 `processor::Processor`。`processor::Processor`类型的全局静态数组PROCESSORS中，每个元素对应一个物理处理器核， 且该类型包含了一个 `ProcessManager`。
可以理解这是一种设计， 这样使得一个进程可以运行在不同的 CPU 上，从而提高多CPU系统的整体利用率。
...
如果PROCESSORS[i]同时加入不同的新进程到 `ProcessManager`，则需要考虑 `ProcessManager`的互斥访问。
...
而 `kmain` (每个 CPU 都会执行 `kmain`， 不管是启动根CPU（第一个启动的CPU），还是附属 CPU (AP)) 中只有一条语句， 实现了所谓的不断选择进程运行的循环。
...
具体而言， `processor.run` 是一个无限循环， 不断执行
1. 调用 `manager.run()`， 选择一个就绪进程
2. 调用底层切换 `swtch` 函数完成进程切换
3. 运行一定时间片段后调用 `manager.stop()` 结束当前进程的运行。

### 抢占调度和 rcore 内核
对于用户进程而言， 它执行的过程中， 随时都有可能产生时钟中断， 然后操作系统让发现用户进程耗尽规定的时间片， 从而导致用户进程被调度出当前CPU。
因此， 用户进程随时都是可能被抢占的。 这体现了用户进程是随时 *可抢占的 (preemptive)*。 通常这也称为， 用户进程是 可*抢占调度的 (preemptively scheduled)*。
...
另外， 调度器需要知道时钟中断的信息。
这是必要的， 因为Ready类型的进程的等待时间可能影响它被选择运行的可能性， 而维护 Running类型的进程所余下的时间片会影响它后续执行的时间。这两种时间信息都需要通过时钟中断来保证及时更新和维护。
因此还需要
* 每次时钟外设产生中断后， 操作系统能通知调度器 "发生了时钟中断"， 以便其更新维护进程中的相关信息。
...
## round-robin 调度算法
round-robin (后称 rr) 算法的思想非常简单: 所有 Ready 的进程平等, 不存在优先级, 轮流使用 CPU。具体调度过程如下：
* rr 将运行队列组织成线性链表
...
## Stride 调度算法的基本思路
Stride 调度算法引入了优先级的概念, 其特点是：
* 进程分配到的 CPU 时间和他的优先级成正比
* 是确定性（即不存在随机调度因素）的调度算法
* 相对容易实现

基本思想是，对于每个进程 $p$ ，具有属性 $p.S \in \mathbb{R}$, 称其 stride。 另外记 $p.P$ 为其优先级。
若 $p$ 被调度运行，则让 $p.S$ 增加 $\frac{1}{p.P}$。
每次选择 stride 最小的 $p$ 运行。

为简化分析，假设在不考虑插入删除进程，不考虑改变优先级的前提下，一个直观的解释是：高优先级的进程 stride 增加更加缓慢。
假设最初所有进程的 $p.S = 0$ 的话, 那么因为高优先级的进程 $p.S$ 增加缓慢, 所以 stride 最小的 $p$ 是高优先级进程的可能更大。这也就实现了 "高优先级进程分配到更多 CPU 时间" 的目标。
...
1. $p.S$ 要用整数模拟, 不能用实数或浮点. 使用 32 位无符号整数表示。
  ...
  - 性质：任一时刻, 任两进程 $a, b$, 有 $|a.S - b.S| < \max\{\frac{B}{x.P}\;|\; x \text{是任一进程}\}$.
    归纳即可证明。
...
为解决溢出问题，可以采用的一个方法是：设 $a, b$ 都是 32 位无符号数，加减运算按照无符号数的规则溢出。
...
另外 stride调度算法需要实现优先队列, 同学们可使用 `alloc::collections::BinaryHeap`。
